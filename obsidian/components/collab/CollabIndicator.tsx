...
export function CollabIndicator({
  ...
  const activeCount = isLocalMode ? 1 : Math.max(1, collaborators.length);

  return (
    <div className="flex flex-col gap-2.5 rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/10 via-background/80 to-purple-500/10 p-3.5 backdrop-blur-xl shadow-lg shadow-primary/5 transition-all">
      <div className="flex flex-wrap items-center justify-between gap-3">
        {/* Left: Connection Status & Mode Badge */}
        <div className="flex items-center gap-2.5">
          <div className="relative flex h-8 w-8 items-center justify-center rounded-xl bg-primary/20 text-primary">
            {isConnected ? (
              <>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5 text-xs text-muted-foreground font-mono bg-background/50 px-2 py-1 rounded-lg border border-border/40">
          ...
          {onLockPaste && !isLocked && (
            <Button
              ...
              variant="outline"
              onClick={onLockPaste}
              className="h-7 text-xs gap-1.5 border-amber-500/30 hover:border-amber-500/60 hover:bg-amber-500/10 text-amber-300"
            >
      ...
      <AnimatePresence>
        {typingUsers.length > 0 && (
          <motion.div
            ...
            className="flex items-center gap-2 pt-1 border-t border-border/30 text-[11px] text-primary font-medium"
          >
